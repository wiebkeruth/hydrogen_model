...
def add_market_variables(model: pyo.ConcreteModel, ppa: PPAConfig) -> None:
    ...
    model.P_nom_wind  = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, ppa.p_nom_wind_max))
    model.P_nom_pv    = pyo.Var(domain=pyo.NonNegativeReals, bounds=(0, ppa.p_nom_pv_max))
...
def apply_market_bounds(model: pyo.ConcreteModel,
    ...
    if not market.use_bess:
        model.P_nom_bess.setub(0)
        print("Market toggle: BESS disabled")
